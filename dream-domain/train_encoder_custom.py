...
for epoch in range(num_epochs):
    ...
    for images, labels in train_dataloader:
        ...
        with torch.no_grad():
            ...
            _, predicted = torch.max(classif_outputs, 1)
        ...
        # encoder_custom: For incorrect images, Cosine similarity should be -1 for true class
        # outputs[incorrect] = - outputs[incorrect]
        # encoder_custom_1: For incorrect images, Cosine similarity should be -1 for pred class
        # outputs[incorrect] = - outputs[incorrect]
        # labels[incorrect] = predicted[incorrect]
        # encoder_custom_2: For incorrect images, Cosine similarity should be 0 for pred class
        # loss = criterion(outputs[incorrect.bitwise_not()], labels[incorrect.bitwise_not()]) + outputs[incorrect, predicted[incorrect]].square().mean()
        # encoder_custom_2bis: For incorrect images, Cosine similarity should be 0 for pred class
        # loss = criterion(outputs[incorrect.bitwise_not()], labels[incorrect.bitwise_not()]) + 0.01*outputs[incorrect, predicted[incorrect]].square().mean()
        # encoder_custom_3: filter out incorrect images
        loss = criterion(outputs[incorrect.bitwise_not()], labels[incorrect.bitwise_not()])
        ...
        # true_classe_cosim[incorrect] = - true_classe_cosim[incorrect] # encoder_custom: Failures should not be similar to anchor
    ...
    save_path = '../results/encoder_custom_3'
