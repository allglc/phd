...
@click.option('--use_classif_reconstruction_loss', is_flag=True, default=False, help='Use the classifier reconstruction loss')
def main(**kwargs):
    ...
    model = Pipeline(config.latent_dim, 2, config.n_samples, config.noise, c_dim=config.c_dim, hidden_dim=config.hidden_dim,
                     ...
                     use_classif_reconstruction_loss=config.use_classif_reconstruction_loss)
    ...
    path_results_exp = path_results / 'pipeline' / (timestamp+tag)
