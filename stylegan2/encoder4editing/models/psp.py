...
class pSp(nn.Module):
    ...
    def __init__(self, opts):
        ...
        if self.opts.stylegan2_implementation == 'rosinality':
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            if self.opts.stylegan2_implementation == 'rosinality': # rosinality's stylegan
                ckpt = torch.load(self.opts.stylegan_weights)
                self.decoder.load_state_dict(ckpt['g_ema'], strict=False)
                self.__load_latent_avg(ckpt, repeat=self.encoder.style_count)
            elif self.opts.stylegan2_implementation == 'nvidia': # nvidia's stylegan
                with open(self.opts.stylegan_weights, 'rb') as f:
                    self.decoder = pickle.load(f)['G_ema']
                self.latent_avg = self.decoder.mapping.w_avg.to(self.opts.device)
