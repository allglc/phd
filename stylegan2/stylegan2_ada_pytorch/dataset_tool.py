...
@click.option('--corruptions', help='Corruption functions to apply', type=str, multiple=True)
@click.option('--corr_max_severity', help='Maximum severity for corruptions', type=int, default=5)
@click.option('--corrupt_proba', help='Probability of applying corruption', type=float, default=1.0)
def convert_dataset(
    ctx: click.Context,
    source: str,
    dest: str,
    ...
    width: Optional[int],
    height: Optional[int],
    corruptions: Optional[list],
    corr_max_severity: Optional[int],
    corrupt_proba: Optional[float]
):
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        # Apply corruption
        if np.random.rand() < corrupt_proba:
            for c in corruptions:
                img = corrupt(img, severity=np.random.randint(1, corr_max_severity+1), corruption_name=c)
